  ...
  <div>
    ...
    <v-data-table :headers="tableHeaders" :items="items" hide-actions :loading="tableLoading">
      ...
      <template slot="no-data">
        {{ tableLoading ? 'Fetching data, please wait...' : 'There are currently no unix block devices.' }}
      </template>
    </v-data-table>
    ...
    <v-dialog v-model="dialog" max-width="700px" scrollable :hide-overlay="linkedItem !== null">
      <v-card flat style="overflow-x:hidden; overflow-y: auto; min-height:calc(100vh - 400px);">
        <v-toolbar card dark color="light-blue darken-3">
          ...
          <v-toolbar-title>{{ editingIndex === -1 ? 'New' : 'Edit' }} Unix Block Device</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items>
  ...
  export default {
    ...
    methods: {
      async initialize () {
        try {
          //
          const response = await axios.get(this.loggedUser.sub + '/api/lxd/devices/unixblock')
        ...
        if (this.$refs.form.validate()) {
          ...
          try {
            ...
            if (this.editingIndex > -1) {
              var response = await axios.put(this.loggedUser.sub + '/api/lxd/devices/unixblock/'+this.editingItem.id, body)
            }
            // add
            else {
              var response = await axios.post(this.loggedUser.sub + '/api/lxd/devices/unixblock', body)
            }
          ...
          buttons: [
            {
              ...
              handler: async () => {
                ...
                try {
                  //
                  const response = await axios.delete(this.loggedUser.sub + '/api/lxd/devices/unixblock/'+item.id)
