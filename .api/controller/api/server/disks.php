 ...
 *
 */
class Disks extends \Base\Controller
{
    /*
    ...
    public function get()
    {
        try {
            if (!$this->cache->exists($this->cache_key, $this->result)) {
                //
                $this->result = $this->system->enumerate(['disk_space', 'total_disk_space', 'disks']);
                $this->result['disks'] = array_values((array) $this->result['disks']);
                //
